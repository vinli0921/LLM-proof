...
class TaskQueue(object):
    def __init__(self, batch_size=512, max_size=1024, name='test'):
        self.name = name
    ...
    def put(self, item):
        with self.not_full:
            while len(self.waiting_list) >= self.max_size:
                self.not_full.wait()
            self.waiting_list.append(item)
    ...
    def get(self, no_wait=False):
        with self.not_empty:
            while len(self.waiting_list) == 0:
                if self.all_tasks_done.is_set():
                    return None
                self.not_empty.wait()
            tasks = self.waiting_list[:self.batch_size]
            del self.waiting_list[:self.batch_size]
    ...
    def close(self):
        ...
        if self._monitor_thread.is_alive():
            ...
            if self._monitor_thread.is_alive():
                ...
                raise RuntimeError("Monitor thread failed to terminate")
...
class ProcessScheduler(object):
    ...
    def close(self):
        """For the scheduler base class"""
        try:
            self.task_queue.close()
        except Exception as e:
            print(f"Error closing task queue: {e}")
            # Continue with cleanup even if task queue close fails
